...
export default function MeetingDetail() {
  ...
  const askAI = async () => {
    ...
    try {
      ...
      // normalize bullets (remove dashes/numbers) and blanks
  ...
  if (loading) return <div className="max-w-3xl mx-auto p-6"><Spinner label="Loading meeting…" /></div>
              ...
              <form onSubmit={submitNewItem} className="mb-4 border rounded p-3 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input className="w-full border rounded px-3 py-2"
                      value={due} onChange={e=>setDue(e.target.value)} placeholder="optional" />
                  </div>
                  <div className="space-y-1">
                    <label className="block text-sm">Assignee</label>
                    <input className="w-full border rounded px-3 py-2"
                      value={assignee} onChange={e=>setAssignee(e.target.value)} placeholder="optional" />
                  </div>
                </div>
                <div className="flex gap-2 justify-end">
                  <button type="button" className="px-3 py-2 border rounded" onClick={()=>{ setShowAdd(false); setTitle(''); setDue(''); setAssignee('') }}>Cancel</button>
